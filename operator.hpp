...
#ifndef RISCV_PARALLEL_OPERATOR_H
#define RISCV_PARALLEL_OPERATOR_H
...
public:
    ...
    instruction ID(){
        ...
        instruction empty;
        if(rs1!=-1&&lock[rs1]==1) {bubble=1;return empty;}//rs1 is locked.return a bubble.向上传气泡
        else{bubble=0;rs1=reg[rs1];}//last time may be a bubble.this time 取值.
        if(rs2!=-1&&lock[rs2]==1) {bubble=1;return empty;}
    ...
    instruction EX(){
        switch(opcode){
            case 55:result=imm;lock[rd]=1;break;//LUI
            case 23:result=imm;lock[rd]=1;break;//AUIPC
            case 111:result=imm;break;//JAL
            case 51: switch(f3){
                     case 0:
            ...
            case 99: switch(f3){
                    ...
                    break;
            case 3:result=rs1+imm;lock[rd]=1;break;//LB&LH&LW&LBU&LHU
            case 19:switch(f3){
                case 0:result=rs1+imm;break;//ADDI
                    ...
                    case 1:result=imm;break;//SLLI
                    case 5:result=imm;break;//SRLI&SRAI
                }
                lock[rd]=1;
                break;
    ...
    void WB(){
        switch(opcode){
            case 55:if(rd!=0)reg[rd]=result;break;//LUI
            case 23:PC+=imm-4;if(rd!=0)reg[rd]=PC;break;//AUIPC
            case 111:if(rd!=0)reg[rd]=PC;PC+=imm-4;PClock=0;break;//JAL
            case 51: if(rd!=0)
                        { switch (f3) {
                            case 0:reg[rd]=result;break;//ADD&SUB
                            case 1:reg[rd]=rs1<<result;break;//SLL
                            case 2:reg[rd]=result;break;//SLT
                            case 3:reg[rd]=result;break;//SLTU
                            case 4:reg[rd]=result;break;//XOR
                            case 5:
                                if (f7 == 0){reg[rd]=(unsigned)rs1>>result;reg[rd]=(int)reg[rd];}//SRL
                                else reg[rd]=rs1>>result;//SRA
                                break;
                            case 6:reg[rd]=result;break;//OR
                            case 7:reg[rd]=result;break;//AND
                        }
            ...
            case 19:if(rd!=0){
                    switch(f3){
                        ...
                        case 1:reg[rd]=(rs1<<result);break;
                        case 5:
                            if(f7==0) reg[rd]=((unsigned)rs1>>result);
                            else reg[rd]=(rs1>>result);
                            break;
                    }
                }
                break;
        ...
        //if(rd==8)std::cout<<opcode<<' '<<f3<<' '<<result<<std::endl;
        if(rd!=-1)lock[rd]=0;
        code=0;
    }
};
#endif //RISCV_PARALLEL_OPERATOR_H
